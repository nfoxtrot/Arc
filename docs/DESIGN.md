...
| Builder audit findings (2026-08-26) not yet acted on | Disabled "Confirm lesson plan" gives no reason why; per-card reorder chevrons could be confused with row-scroll controls now that Start/End share the row |
...
- **2026-08-26:** Acted on the three remaining full-app audit findings, all in `SignupSheet`:
  - Added a password-visibility toggle (eye icon) to both Password and Confirm password, each with independent reveal state so showing one doesn't show the other.
  - Demoted "Continue with Google" from `FilledTonalButton` to `OutlinedButton` — a deliberate, flagged exception to the "every medium-emphasis button is tonal" sweep from earlier the same day. The fully-colored tonal button visually outweighed the primary "Let's get started" button above it whenever the form was still invalid (disabled/gray); outlined keeps it clearly secondary regardless of the primary's state.
  - Gave "Skip for now" an explicit `onSurfaceVariant` content color instead of `TextButton`'s default (`colorScheme.primary`, confirmed from the real M3 token source) — that default made a low-emphasis dismiss action use the same color that signals "the enabled primary action" everywhere else.
  - Verified all three on the emulator in both themes, including the enabled-form state (solid primary next to the now-outlined secondary) and a full reveal/hide round-trip on both password fields actually reading the live field value, not just the icon swapping.
  - Verification note: several automation missteps along the way (misjudging a mismatch as a bug, misreading a dot count, a stray tap that appended text into the wrong field) were all traced back to imprecise `adb` coordinates or unverified assumptions about field contents, not real app bugs — resolved each by reading the actual revealed text or dumped bounds rather than trusting a screenshot glance.
- **2026-08-26:** Fixed "Suggest chapters for me" giving zero feedback on tap, reported directly ("I don't see anything load or change"). It was a true no-op (`onClick = { /* comment only */ }`) — indistinguishable from broken. Added a `SnackbarHostState` at the `AppShell` level (Scaffold's `snackbarHost`), passed an `onSuggestChapters` callback down into `BuilderScreen`, and wired the button to show "AI chapter suggestions aren't built yet" — still a stub, but now honestly visible as one instead of silent. Also closed the companion audit finding from the same day in the same pass: the button was enabled before Start/End were filled in, which never made sense for what it will eventually do; now gated on both being non-blank (`!isLocked && startTopic.isNotBlank() && endTopic.isNotBlank()`). Verified live: disabled with blank Start/End, enabled once both are filled, and the Snackbar text confirmed both visually and via a fresh `uiautomator dump` taken immediately after the tap (a screenshot alone risked missing it — Snackbars auto-dismiss and a `screencap`+`pull` round-trip is slower than the dump).
